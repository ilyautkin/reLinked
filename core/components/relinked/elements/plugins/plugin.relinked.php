...
switch ($modx->event->name) {
    case 'OnWebPagePrerender':
        ...
        if ($links = $modx->getCollection('rldLink', array('resource' => $modx->resource->get('id')))) {
            ...
            $templates = $modx->resource->getProperty('positions', 'relinked');
            if (!$templates) {
                $templates = array();
            }
            ...
            $modx->resource->setProperty('positions', $templates, 'relinked');
            $modx->resource->save();
    ...
    case 'OnDocFormRender':
        ...
        /** @var reLinked $reLinked */
		if ($mode != 'new' && $reLinked = $modx->getService('reLinked', 'reLinked', $modx->getOption('relinked_core_path', null, $modx->getOption('core_path') . 'components/relinked/') . 'model/relinked/')) {
            $reLinked->loadManagerFiles($modx->controller, $resource);
        }
		break;
    default:
        break;
